...
Page({
  ...
  onShow() { const state = store.getState(); this.setData({ addresses: state.addresses.map((item) => Object.assign({}, item, { isDefault: item.id === state.addressId })), editing: false }) },
  syncRegion(form) {
  ...
  save() { const form = this.data.form, regionCodes = regionCodesFor(form); if (!this.data.regionKnown || !regionCodes) return wx.showToast({ title: '请重新选择所在地区后保存', icon: 'none' }); if (!form.contactName || !form.phone || !form.province || !form.city || !form.district || !form.detail) return wx.showToast({ title: '请完善收货地址', icon: 'none' }); if (!validPhone(form.phone)) return wx.showToast({ title: '请输入11位手机号', icon: 'none' }); store.saveAddress(Object.assign({}, form, regionCodes)); this.onShow(); wx.showToast({ title: '地址已保存' }) },
  choose(e) { store.setSelectedAddress(e.currentTarget.dataset.id); wx.showToast({ title: '已选择收货地址', icon: 'none' }); setTimeout(() => wx.navigateBack(), 250) },
  remove(e) { const id = e.currentTarget.dataset.id, state = store.getState(); if (id === state.addressId && state.addresses.length > 1) return wx.showToast({ title: '请先选择其他默认地址', icon: 'none' }); store.removeAddress(id); this.onShow() },
