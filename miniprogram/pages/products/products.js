...
Page({
  ...
  onShow() { productRepository.list().then((result) => { this.catalogProducts = result.items || []; this.sync() }).catch(() => this.sync()) },
  sync() { const items = store.getCart('pickup'), products = this.catalogProducts || [], source = this.data.activeCategory === '全部' ? products : products.filter((item) => item.category === this.data.activeCategory); this.setData({ products: source.map((product) => { const item = quickItem(product), current = items.find((entry) => entry.id === item.id), unavailable = product.soldOut || product.supportsPickup === false; return Object.assign({}, product, { priceText: formatFen(product.priceFen), quantity: current ? current.quantity : 0, unavailable }) }), cartCount: cart.count(items), cartSubtotal: formatFen(cart.subtotal(items)), store: store.getSelectedStore() }) },
