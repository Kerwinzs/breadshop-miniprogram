...
Page({
  data: { order: null, canCancel: false, canReorder: false },
  onLoad(q) { this.id = q.id },
  onShow() { const local = normalize(store.getOrder(this.id)); this.setData({ order: local, canCancel: !!local && local.orderStatus === '已下单', canReorder: !!local && local.orderStatus === '已完成' }); remote.get(this.id).then((result) => { const order = normalize(result.order); store.updateOrder(this.id, result.order); this.setData({ order, canCancel: order.orderStatus === '已下单', canReorder: order.orderStatus === '已完成' }) }).catch(() => {}) },
