...
function getOrder(orderId) { return getOrders().find((order) => order.id === orderId) || null }
function updateOrder(orderId, partial) { const state = getState(); const orders = state.orders.map((order) => order.id === orderId ? Object.assign({}, order, partial || {}) : order); saveState(Object.assign({}, state, { orders })); return orders.find((order) => order.id === orderId) || null }
